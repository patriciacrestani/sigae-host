                    ...
                    <div class="flex flex-column items-center p-4 bg-surface-0 dark:bg-surface-900 h-full" style="border-radius: 8px">
                        ...
                        <div class="block ml-6">
                            <h5 class="mb-2 text-surface-900 dark:text-surface-0">
                                <span class="text-2xl">{{ qtdEscolas }}</span> escolas cadastradas!
                            </h5>
                            <a class="text-surface-600 dark:text-surface-200" href="/cadastros/escolas">Ir para escolas <i class="pi pi-external-link !text-sm pl-1"></i></a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-span-2 md:col-span-1 p-0 lg:pr-8 lg:pb-8 mt-6 lg:mt-0">
                <div style="padding: 2px; border-radius: 10px; background: linear-gradient(90deg, rgba(145, 226, 237, 0.2), rgba(172, 180, 223, 0.2)), linear-gradient(180deg, rgba(172, 180, 223, 0.2), rgba(246, 158, 188, 0.2))">
                    <div class="flex flex-column items-center p-4 bg-surface-0 dark:bg-surface-900 h-full" style="border-radius: 8px">
                        ...
                        <div class="block ml-6">
                            <div class="mb-2 text-surface-900 dark:text-surface-0 text-xl font-semibold">
                                <span class="text-2xl">{{ qtdPessoas }}</span> pessoas cadastradas!
                            </div>
                            <a class="text-surface-600 dark:text-surface-200" href="/cadastros/pessoas">Ir para pessoas <i class="pi pi-external-link !text-sm pl-1"></i></a>
                        </div>
                    </div>
                </div>
            </div>
...
export class FeaturesWidget implements OnInit {
    ...
    async ngOnInit() {
        await this.obterTotais();
    }

    async obterTotais() {
        await this.obterQtdEscolas();
        await this.obterQtdPessoas();
    }

    async obterQtdEscolas() {
        const total = await this.escolaService.obterTotalDeEscolas();
        this.qtdEscolas = total;
    }

    async obterQtdPessoas() {
        const total = await this.pessoaService.obterTotalDePessoas();
        this.qtdPessoas = total;
    }
}
